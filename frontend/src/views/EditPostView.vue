...
async function loadPost() {
  try {
    ...
    const postOut = await postsAPI.get(id)
    if (postOut.Post.user_id !== auth.currentUser?.id) {
      router.push(`/posts/${id}`)
      return
    }
    title.value = postOut.Post.title
    content.value = postOut.Post.content
    published.value = postOut.Post.published
  } catch {
    router.push('/')
  }
...
async function handleSubmit() {
  ...
  try {
    ...
    if (err && typeof err === 'object' && 'response' in err) {
      ...
      error.value = axiosErr.response?.data?.detail || 'Failed to update post'
    } else {
...
<template>
  <div class="max-w-3xl mx-auto px-4 py-8">
    <div v-if="fetching" class="animate-pulse space-y-4">
      <div class="h-8 bg-slate-700 rounded w-1/3 mb-8"></div>
      <div class="h-12 bg-slate-700 rounded w-full"></div>
      <div class="h-48 bg-slate-700 rounded w-full"></div>
    </div>

    <div v-else class="bg-slate-800 rounded-2xl border border-slate-700 p-8 shadow-2xl">
